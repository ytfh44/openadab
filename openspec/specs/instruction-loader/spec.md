...
The `InstructionLoader` module SHALL assemble a complete instruction payload for an artifact by combining project config rules, schema artifact definition, template content (with variable interpolation), dependency artifact contents, and context pack reference.
...
- **THEN** the output SHALL include:
  ...
  - `template`: the rendered template content (if `template` specified in schema)
  - `outputPath`: the resolved file path where the artifact should be written
  - `contextPack`: reference to the context pack produced by the ContextPacker (file lists with reasons)
  - `rules`: per-artifact rules from project config
  - `requiredReads`: list of dependency file paths (always included)
  - `dependencyContent`: full text content of all dependency artifacts (only if `--inline-deps` flag)
...
- **AND** `{{config.*}}` variables SHALL be resolved against the project configuration
...
- **THEN** the output SHALL include the full text content of each dependency artifact file (e.g., `brief.md`, `scene-plan.md` for `draft`)

#### Scenario: Dependency content excluded by default
- **WHEN** loading instructions without `--inline-deps`
- **THEN** the output SHALL only reference dependency file paths in `requiredReads`
- **AND** `dependencyContent` SHALL be omitted or empty
...
- **AND** the `contextPack` SHALL include `mustRead`, `optionalRead`, `excluded`, and `reasons` objects
