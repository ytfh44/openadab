...
- **AND** SHALL detect cycles in both artifact dependencies and `apply.requires` (which is treated as a node "apply" in the graph)

#### Scenario: Duplicate artifact IDs
- **WHEN** the engine loads a schema with two artifacts having the same `id` field
- **THEN** it SHALL report a `SchemaValidationError` indicating the duplicate ID (as a validation result error, not a throw)

#### Scenario: Unknown dependency references
- **WHEN** `apply.requires` references an artifact ID not defined in the schema
- **THEN** it SHALL report the error in the validation result
...
- **THEN** the engine SHALL throw a `TemplateNotFoundError` with the resolved path (during validation)
...
- **AND** SHALL copy all template files recursively from `chapter-draft/templates/` to `adab/schemas/my-custom-draft/templates/`
- **AND** SHALL add metadata `forked_from: chapter-draft` and `forked_version: <current-cli-version>` to the copied schema
- **AND** the CLI version SHALL be read from the nearest `package.json` relative to the module, falling back to `0.1.0`

#### Scenario: Fork non-existent base schema
- **WHEN** user runs `openadab schema fork nonexistent my-copy`
- **THEN** the CLI SHALL error with "Schema 'nonexistent' not found. Available: <list>"
- **AND** available schemas SHALL be listed from the built-in schemas directory
...
- **AND** generic interpolation uses `\w+` pattern to match variable names
...
- **AND** nested config paths use dot notation (e.g., `{{config.project.title}}`)
...
- **AND** missing config variables (null or undefined) SHALL also throw `UnresolvedVariableError`
