 ...
 *
 * Returns `null` if the file does not exist.  Any other error (permission
 * denied, directory not readable, etc.) is wrapped in an {@link AdabError}
 * and thrown.
 *
 * @param path Absolute or relative path to the file.
 * @returns File contents as UTF-8 string, or `null` when missing.
 */
...
export async function atomicWriteFile(path: string, content: string): Promise<void> {
  ...
  try {
    ...
    const e = err as NodeJS.ErrnoException;
    throw new AdabError(`Failed to write "${path}": ${e.message}`, 'FS_WRITE_ERROR', { cause: err });
  }
}
...
export async function ensureDir(path: string): Promise<void> {
  try {
    ...
    const e = err as NodeJS.ErrnoException;
    throw new AdabError(`Failed to create directory "${path}": ${e.message}`, 'FS_MKDIR_ERROR', { cause: err });
  }
}
