...
describe('sync → archive pipeline', () => {
    ...
    it('24.6 full change completion updates manuscript, archives change, and logs', async () => {
        ...
        await writeFile(join(changeDir, 'wiki-diff.md'), `---\nchangeId: ${changeId}\n---\n\n### [[characters/mara]]\n\nSource: ${changeId}/continuity-report.md\n\n#### Add to Current State\n\n- Ready to steal.\n`, 'utf-8');
        // Sync
    ...
    it('24.6 edge: sync with empty wiki-diff still updates manifest and indexes', async () => {
        ...
        const syncEngine = new SyncEngine(projectRoot, new WikiDiffParser(), new WikiDiffApplier(projectRoot, wikiEngine), wikiEngine, mentionIndexer, progressionTracker, contextPacker, validator2);
        const syncReport = await syncEngine.sync(changeId);
...
//# sourceMappingURL=sync-archive-pipeline.test.js.map